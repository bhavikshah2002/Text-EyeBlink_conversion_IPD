...
while True:
	...
	for rect in rects:
		...
		cv2.putText(frame, "Morse_Code: {}".format(TOTAL), (10, 60),
			cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
		cv2.putText(frame, "EAR: {:.2f}".format(ear), (400, 60),
			cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
		cv2.putText(frame,"Messsage: {}".format(string), (20,130), 
			cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
 
	# show the frame
	cv2.imshow("Frame", frame)
	key = cv2.waitKey(1) & 0xFF
	# if the `ESC` key was pressed, break from the loop. Here is key value of ESC.
	if key == 27:
		break
